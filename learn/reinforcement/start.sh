...
nohup python3 -u -m learn.reinforcement.learn --proc_name $PROCESS_NAME --input_path $OUTPUT_PATH --device "cuda:1" --lr 0.0000001 >> $OUTPUT_PATH/logs/learn.txt < /dev/null &

for ((i=0; i<$PROCESS_NUM; i++)) do
    nohup python3 -u -m learn.reinforcement.battle --proc_name $PROCESS_NAME --output_path $OUTPUT_PATH --temperature 0.01 >> $OUTPUT_PATH/logs/battle_$i.txt < /dev/null &
done
