...
parser = argparse.ArgumentParser(description="ポリシーとvalueを1epoch学習する")
...
parser.add_argument("--policy_output_path", type=str,
                    default=None, help="ポリシーを記録するパス")
parser.add_argument("--value_output_path", type=str,
                    default=None, help="バリューを記録するパス")
...
with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_num) as executor:
    ...
    torch.save(learner.state_dict(), args.policy_output_path)
    ...
    for feature_data, move_data, reward_data in dataloader:
        optimizer.zero_grad()
        outputs = value(feature_data).cuda()
        loss = loss_fn(outputs, reward_data).cuda()
        loss = loss.mean()
        loss.backward()
        optimizer.step()

    torch.save(value.state_dict(), args.value_output_path)