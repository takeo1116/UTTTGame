...
def train():
    ...
    for feature_tensor, move_tensor, value_tensor in train_dataloader:
        ...
        loss_p = loss_fn_p(p, move_tensor).cuda()
        loss_v = loss_fn_v(v, value_tensor).cuda()
...
parser.add_argument("--init_model", type=str, default=None, help="初期モデルのパス")
...
os.makedirs(os.path.join(args.output_path, "policy_graphs"), exist_ok=True)
os.makedirs(os.path.join(args.output_path, "value_graphs"), exist_ok=True)
...
if args.init_model is not None:
    ...
    print(f"model {args.init_model} loaded")
...
for epoch in range(args.epoch):
    ...
    if epoch % 1 == 0:
        model_path = f"{args.output_path}/models/state_{epoch + 1}.pth"
        ...
        fig.savefig(f"{args.output_path}/policy_graphs/img_{epoch + 1}.png")
        ...
        fig.savefig(f"{args.output_path}/value_graphs/distribution_{epoch + 1}.png")
